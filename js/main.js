...
function getHours1(format) {
  ...
  return `Qiyosh chiqish${String(date.getHours()).padStart(
    2,
    "0"
  )}:${date.getMinutes()}`;
}
...
function theAir(arr, node) {
  [arr].forEach((e) => {
    ...
    newTime.textContent = getDate(e.timezone);
...
async function getTheAir() {
  let response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${elInputVal}&appid=d7e0c21cccd15a5f478c11c35c0bfcc6&units=metric`
  );
